const API_KEY = 'AIzaSyDlWQLBUm53PWCdQIbgQca8TA9jWGQL-l8';
...
const createListVideo = (videos, titleText, pagination) => {
  ...
  const listVideos = videos.items.map(video => {
    ...
    <article class="video-card">
      <a class="video-card__link" href="#/video/id=${id}">
        ...
        ${video.contentDetails ? `<p class="video-card__duration">${convertISOToReadableDuration(video.contentDetails.duration)}</p>` : ""
        }

      </a>

      <button class="video-card__favorite favorite ${favoriteIds.includes(id) ? 'active' : ''}" 
        type="button" 
        aria-label="Добавить в избранное, ${video.snippet.title}" data-video-id="${id}">
  ...
  if(pagination) {
    ...
    if(pagination.prev) {
      ...
      arrowPrev.textContent = 'Предыдущая стр';
    ...
    if(pagination.next) {
      ...
      arrowNext.textContent = 'Следующая стр';
  ...
  videoSection.innerHTML = `
    <div class="container">
      <div class="video__player">
        <iframe class="video__iframe" src="https://www.youtube.com/embed/${video.id}" frameborder="0" allowfullscreen 
...
const indexRoute = async () => {
  ...
  const videos = await fetchTrendingVideo();
...
const favoriteRoute = async () => {
  ...
  const search = createSearch();
  const videos = await fetchFavoriteVideo();
  preload.remove();
